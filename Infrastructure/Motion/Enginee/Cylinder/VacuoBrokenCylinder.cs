    ...
    public class VacuoBrokenCylinder : Cylinder, ICylinderStatusJugger
    {
        ...
        public IList<Alarm> Alarms
        {
            get
            {
                ...
                list.Add(new Alarm(() => NoSensorAlarm()) { External = Condition.External, AlarmLevel = AlarmLevels.Error, Name = string.Format("{0}无感应信号报警！", Name) });
                list.Add(new Alarm(() => OffSensorAlarm()) { External = Condition.External, AlarmLevel = AlarmLevels.Error, Name = string.Format("{0}感应信号常量报警！", Name) });
