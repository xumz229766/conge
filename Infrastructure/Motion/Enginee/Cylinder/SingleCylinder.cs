    ...
    public class SingleCylinder : Cylinder, ICylinderStatusJugger
    {
        ...
        public IList<Alarm> Alarms
        {
            get
            {
                ...
                list.Add(new Alarm(() => NoOriginAlarm()) { External = Condition.External, AlarmLevel = AlarmLevels.Error, Name = string.Format("{0}无原点报警！", Name) });
                list.Add(new Alarm(() => NoMoveAlarm()) { External = Condition.External, AlarmLevel = AlarmLevels.Error, Name = string.Format("{0}无动点报警！", Name) });
                list.Add(new Alarm(() => OnOriginAlarm()) { External = Condition.External, AlarmLevel = AlarmLevels.Error, Name = string.Format("{0}ON时，原点信号报警！", Name) });
                list.Add(new Alarm(() => OffMoveAlarm()) { External = Condition.External, AlarmLevel = AlarmLevels.Error, Name = string.Format("{0}OFF时，动点信号报警！", Name) });
                list.Add(new Alarm(() => AutoOnPrompt()) { External = Condition.External, AlarmLevel = AlarmLevels.None, Name = string.Format("{0}自动时，手动为ON提示！", Name) });
                return list;
            }
        }
